...
Skip formal coordination for one bounded task or an obvious direct fan-out of independent calls with no meaningful delivery boundary, dependency, collision, worktree isolation, approval gate, or integration question; launch those calls directly in parallel.

Otherwise, use formal coordination before the first subagent call when the work has one or more of these characteristics:

- distinct features or independently deliverable outcomes whose delivery boundaries need a decision
...
- concurrent implementation that may require worktree isolation
...
1. **Outcomes and delivery boundaries** — Name each distinct feature or independently deliverable outcome and its acceptance invariant. Give each separate outcome its own delivery boundary across its branch, worktree, and pull request; keep one outcome's implementation, tests, and supporting documentation together.
...
6. **Worktree isolation** — Keep read-only streams in the current checkout when safe. Use isolated worktrees for concurrent write streams or separate delivery boundaries, and define how their results will be integrated.
7. **Approval gates** — Mark every applicable approval gate, including approval gates for edits under strict guidance and any user- or repository-required approval gates for destructive operations, external writes, commits, pushes, or pull-request changes. Coordination never opens an approval gate.
