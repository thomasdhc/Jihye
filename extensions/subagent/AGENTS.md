...
- Agent directories are merged in the order `personas/subagents` (package), `<pi agent dir>/agents` (user), `<package root>/.pi/agents` (package-local, gitignored); later wins. The merge replaces the whole agent by name, not field by field. `getAgentDirectories` takes no arguments, so the set does not depend on the tool call's working directory; project-local discovery is deliberately absent.
- `PI_SUBAGENT_ALLOWED` is read once at module load into `SUBAGENT_ALLOWLIST` in `discovery.ts` and applied only through `isAgentAllowed`, at three sites: `registerAgent` drops non-allowlisted registrations, and `index.ts` filters the directory scan both at startup and per `execute`. Unset, empty, and whitespace-only values all mean no restriction. A child therefore cannot spawn an agent its parent did not grant, even by name; add new checks through the helper rather than a fourth `includes` test.
- `runner.ts` owns the child-process contract:
  - The child runs `--mode json -p --no-session --no-skills --no-extensions` but never `--no-context-files`, so global, workspace, and repository guidance remains loaded. It restores bundled `jihye-setup` as prompt infrastructure, then gets only the tools its definition declares — built-ins via `--tools`, custom tools via explicit `--extension` paths. `bash` implies loading `bash-guard`.
...
2. Prefer baseline `model_tier` (`standard` or `deep`) over a pinned `model`. Omit `provider_strategy` to follow the parent provider; when independent provider perspective is part of the role, add `provider_strategy: alternate` and optionally `alternate_model_tier`. The strategy activates only when local config sets `enableAlternateProviders: true`. A pinned model always wins.
