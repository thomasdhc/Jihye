...
- `runner.ts` owns the child-process contract:
  - The child runs `--mode json -p --no-session --no-skills --no-extensions`, always restores bundled `jihye-setup` as prompt infrastructure, then gets back only the tools its definition declares — built-ins via `--tools`, custom tools via explicit `--extension` paths. `bash` implies loading `bash-guard`.
