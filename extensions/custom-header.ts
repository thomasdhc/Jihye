...
function buildHeader(theme: Theme): string {
	...
	// 3D effect: success (green) = front face blocks, accent (blue) = depth edges
	const f = (s: string) => theme.bold(theme.fg("success", s)); // front face
	const e = (s: string) => theme.bold(theme.fg("accent",  s)); // depth edge
	const eyeUnit = theme.bold("█") + theme.fg("dim", "▌");
